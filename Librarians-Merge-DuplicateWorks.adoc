...
The Migrate Editions Tool requries link:https://github.com/internetarchive/openlibrary/wiki/Writing-Bots[bot privileges]. If you do not have or want bot privileges, please submit a merge request instead.
...
TIP: The magic merge wand only works on one search page. All selections are forgotten if you switch to another search page. If you want to merge duplicate work profiles from different search result pages, you need to either do it iteratively, manually or tweak your search (see: Search documentation).
